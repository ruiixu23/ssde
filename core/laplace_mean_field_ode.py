...
class LaplaceMeanFieldODE(LaplaceMeanField):
    def __init__(self, dynamical, config, gp):
        super(LaplaceMeanFieldODE, self).__init__(dynamical, config, gp)
    ...
    def init_optimization(self):
        ...
        for i in range(self.dynamical.num_x):
            self.F[i] = self.dynamical.F_funcs[i](self.eta_X, self.eta_theta)
    ...
    def optimize_theta(self):
        ...
        for i in range(self.dynamical.num_x):
            for n in range(self.dynamical.num_theta):
                self.theta_B[i, :, n] = self.dynamical.theta_B_funcs[i][n](self.eta_X)
            self.theta_b[i] = self.dynamical.theta_b_funcs[i](self.eta_X)
    ...
    def x_objective_and_gradient(self, x):
        ...
        if self.dynamical.x_F_relation[i]:
            for j in self.dynamical.x_F_relation[i]:
                self.F[j] = self.dynamical.F_funcs[j](self.eta_X, self.eta_theta)
        ...
        for j in self.dynamical.x_dF_relation[i]:
            ...
            self.x_gradients[i] += self.dynamical.x_dF_funcs[i][j](self.eta_X, self.eta_theta) * self.Lambda_F_mx[j]
    ...
    def x_Hessian(self, x):
        ...
        for j in self.dynamical.x_ddF_relation[i]:
            self.x_Hessians[i][self.diagonal_indices] += (
                self.dynamical.x_ddF_funcs[i][j](self.eta_X, self.eta_theta) * self.Lambda_F_mx[j]
            )

        if self.dynamical.x_dF_relation[i]:
            for j in self.dynamical.x_dF_relation[i]:
                tmp = self.dynamical.x_dF_funcs[i][j](self.eta_X, self.eta_theta)
                if j == i:
